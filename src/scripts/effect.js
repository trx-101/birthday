...
$('document').ready(function(){
	...
	// 	});
	// });
	// Listen for a click event on the element with id "initial"
$("#initial").on("click", function () {
	const pass =10111100110;
	Swal.fire({
		title: 'Enter Your DOB',
		input: 'password',
		inputAttributes: {
		  autocapitalize: 'off',
		  placeholder: 'Hint: if DOB - 20/10/2000, then put DOB: 2010'
		},
		...
		preConfirm: (password) => {
			var pas = parseInt(password).toString(2);
		  if (pas === pass.toString()) {
			// Password is valid, perform actions
			Swal.fire({
			  icon: 'success',
			  title: '😊😊 Happy birthday to someone who is ForeEver Young 😊😊',
			  text: 'You can proceed now.'
			}).then(() => {
			  // Fade out the element with id "initial"
			  $(this).fadeOut('slow').delay(50).promise().done(function () {
				// Fade in the element with id "turn_on"
				$('#turn_on').fadeIn('slow');
			  });
			});
		  } else {
			Swal.fire({
			  ...
			  text: 'Please try again. Hint: Only Put Date and Month only'
			});
		  }
		},
	...
	$('#play').click(function(){
		...
		$(this).fadeOut('slow').delay(5000).promise().done(function(){//5000
...
debugger;
	...
	$('#dj_time').click(function(){
		...
		$(this).fadeOut('slow').delay(3000).promise().done(function(){//5000
			$('#story').fadeIn('slow');
		});
	});
